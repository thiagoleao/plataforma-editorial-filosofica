...
- **Débito técnico (qualidade de extração, 2026-07-15):** usuário validou uma amostra de segmentos/fichas gravados pelo Fluxo 02 e considerou o conteúdo raso, mal estruturado e sem uma "rede de pensamento" emergindo do grafo de conceitos (ADR-011/012). Três frentes levantadas, ainda sem ADR: (1) melhorar a extração/síntese das fichas (português higienizado, fluidez, começo-meio-fim do pensamento) — só se aplica à camada de fichas, nunca aos segmentos literais (ADR-010); (2) enriquecer `concept_relations` além de co-ocorrência/similaridade — relações tipadas (causa, contraste, pré-requisito) em vez de só "relacionado"; (3) separar conteúdo específico de "chamas gêmeas" de conceitos universais que aparecem nessas sessões mas valem para qualquer leitor, sem ficar restritos ao rótulo do tema — provavelmente via um atributo de escopo/categoria em `editorial.concepts` (universal vs. temático), não necessariamente uma base física separada. Ver troca completa no histórico da conversa de 2026-07-15. Próximo passo: desenhar isso como ADR nova (síntese de fichas) e/ou aditivo à ADR-011 (escopo de conceito), antes de qualquer reprocessamento em massa das ~76 fichas existentes.
