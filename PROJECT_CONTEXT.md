...
**Próxima a implementar (2026-07-16):**
- [ADR-023](./docs/adr/ADR-023.md) — Fechar o Backlog de Destilação e Expor o Valor do Acervo. **Proposta, priorizada à frente da ADR-015 por decisão explícita do usuário.** Motivada por uma investigação direta nos dados de produção: só 23 de 172 sessões transcritas (13%) já passaram pelo Fluxo 02, 0 capítulos chegaram a `reviewed`/`final`, 0 relações tipadas em 1648 (efeito colateral da geração de cartão de insight, usada só 5 vezes), e não existe nenhuma tela para navegar o acervo fora do fluxo de montagem de capítulo. Decisão: (1) aumentar a vazão do Fluxo 02 (hoje 1 arquivo/execução); (2) nova tela "Explorar Acervo" na `editorial-ui`, fora do `ChapterBuilder`; (3) geração de cartão de insight em lote, mas sob controle explícito (não automática — preserva a cautela de custo da ADR-022 §2); (4) remover projeto de teste esquecido em produção. Ver a ADR para os números completos do levantamento.

**Despriorizada (2026-07-16):** [ADR-015](./docs/adr/ADR-015.md) — Publicação Final (DOCX/PDF/EPUB). Continua "Proposta", mas o usuário decidiu explicitamente adiá-la — não faz sentido investir em exportação final antes do acervo/capítulos terem conteúdo real percorrendo o pipeline até o fim, e o desenho precisaria ser revisto em cima do manuscrito (ADR-020) de qualquer forma (a ADR-015 foi escrita antes da ADR-020 existir).
...
Ver seção "Roadmap de ADRs propostas" acima. Próximo passo de implementação: **ADR-023** (Fechar o Backlog de Destilação e Expor o Valor do Acervo) — prioriza vazão e visibilidade do que já existe sobre features novas. ADR-015 (Publicação Final) fica despriorizada até o acervo/capítulos terem conteúdo real percorrendo o pipeline até o fim.
...
│       ├── ADR-023.md          # PROPOSTA (próxima a implementar) — vazão do Fluxo 02, tela Explorar Acervo, insight em lote controlado
